...
export function TrendChart({ candles, result, signals, range }: TrendChartProps) {
  ...
  useEffect(() => {
    ...
    const chart: IChartApi = createChart(chartContainer, { width: chartContainer.clientWidth, height: 410, layout: { background: { type: ColorType.Solid, color: isLight ? "#ffffff" : "#0e1726" }, textColor: isLight ? "#65738b" : "#99a8c2" }, grid: { vertLines: { color: isLight ? "#e1e8f2" : "#1c2a3e" }, horzLines: { color: isLight ? "#e1e8f2" : "#1c2a3e" } }, rightPriceScale: { borderColor: isLight ? "#cfd9e7" : "#2a3950" }, timeScale: { borderColor: isLight ? "#cfd9e7" : "#2a3950", timeVisible: false }, crosshair: { vertLine: { color: isLight ? "#8492a8" : "#61708a" }, horzLine: { color: isLight ? "#8492a8" : "#61708a" } } });
  ...
  }, [candles, range, result.points, resolvedTheme, signals]);
