...
describe("Coinbase adapter", () => {
  it("normalizes sorted candles and excludes the forming daily bucket", async () => {
    const result = await fetchHistoricalCandles("BTC-USD", { fetchFn: fakeFetch(rows().reverse()), now: NOW, startTimestamp: START });
  ...
  it("fails closed for a malformed response", async () => {
    await expect(fetchHistoricalCandles("BTC-USD", { fetchFn: fakeFetch({ candles: [] }), now: NOW, startTimestamp: START })).rejects.toBeInstanceOf(CoinbaseDataError);
  });
  it("fails closed for missing daily data and API errors", async () => {
    const missing = rows().filter((_, index) => index !== 90);
    await expect(fetchHistoricalCandles("ETH-USD", { fetchFn: fakeFetch(missing), now: NOW, startTimestamp: START })).rejects.toBeInstanceOf(CoinbaseDataError);
    await expect(fetchHistoricalCandles("ETH-USD", { fetchFn: fakeFetch({}, 500), now: NOW, startTimestamp: START })).rejects.toBeInstanceOf(CoinbaseDataError);
  });
  it("rejects duplicate timestamps", async () => {
    const duplicate = rows(); duplicate.push(duplicate[0]);
    await expect(fetchHistoricalCandles("BTC-USD", { fetchFn: fakeFetch(duplicate), now: NOW, startTimestamp: START })).rejects.toBeInstanceOf(CoinbaseDataError);
  });
});
